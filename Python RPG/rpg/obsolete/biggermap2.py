...
class map(pygame.sprite.Sprite):
	...
	def load_player_section(self):#this will load the player's area based on what 
		...
		#left/right viewbox is 256-544
		#top/bottom viewbox is 224-416
		
		if self.xtilestart > 9 and self.xtilestart < xwindow-25:#25-8
			...
			self.left_viewbox = 0
			tilex = 0
			if self.player_object.rect.x <= 0:
				if self.player_object.hspeed < 0:
					self.player_object.rect.x = self.left_viewbox + 32
		
		elif self.xtilestart < 10:
			self.left_viewbox = 0
			tilex = 0
			self.xtilestart = 0
			if self.player_object.rect.x <= self.left_viewbox:#If player is hitting left viewbox, go left.
		...
		elif self.xtilestart == (xwindow - 25):
			self.right_viewbox = 800
			tilex = xwindow-25
			if self.player_object.rect.x <= 0:
				if self.player_object.hspeed < 0:
					self.player_object.rect.x = self.right_viewbox + 32
		
		elif self.xtilestart > (xwindow - 25):
			self.right_viewbox = 800
			tilex = xwindow-25
			self.xtilestart = xwindow-25
			if self.player_object.rect.x <= self.right_viewbox:#If player is hitting left viewbox, go left.
				self.player_object.rect.x = self.right_viewbox
